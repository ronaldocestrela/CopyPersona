...
public class HeuristicClarificationAnalyzer
{
    ...
    {
        var answer = dto.PorQueEscolhemVoce ?? dto.DiferencialAtendimento ?? string.Empty;
        if (IsVagueOrShort(answer, 20, GenericClichesEtapa3))
        {
            items.Add(new ClarificationItemDto(
                QuestionId: "3.5",
                FieldName: nameof(dto.PorQueEscolhemVoce),
                CurrentAnswer: answer,
                ReasonVague: "Sua resposta é genérica ou muito curta. Termos como 'humanizado' ou 'dedicado' são esperados por qualquer cliente.",
    ...
    {
        var answer = dto.MaioresMedos ?? dto.MaioresDesejos ?? string.Empty;
        if (IsVagueOrShort(answer, 25, GenericClichesEtapa4))
        {
            items.Add(new ClarificationItemDto(
                QuestionId: "4.2",
                FieldName: nameof(dto.MaioresMedos),
                CurrentAnswer: answer,
                ReasonVague: "A resposta é genérica. Para gerar roteiros de alto engajamento, precisamos da dor ou desejo exato do paciente.",
                SuggestionTitle: "Especifique a dor/desejo emocional do seu cliente",
    ...
    {
        var answer = dto.VerdadeCorajosa ?? string.Empty;
        if (IsVagueOrShort(answer, 20, GenericClichesEtapa7))
        {
            items.Add(new ClarificationItemDto(
                ...
                CurrentAnswer: answer,
                ReasonVague: "Sua resposta sobre sua metodologia ou opinião forte é muito curta.",
    ...
    {
        var answer = dto.AmostraEscritaExplicativa ?? string.Empty;
    ...
    private static bool IsVagueOrShort(string text, int minLength, string[] clichês)
    {
